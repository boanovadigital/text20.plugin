...
public class SessionRecorderManagerImpl implements SessionRecorderManager {
    ...
    /** List with all session recorder */
    final List<SessionRecorderImpl> allRecorder = new ArrayList<SessionRecorderImpl>();

    /* (non-Javadoc)
     * @see de.dfki.km.text20.browserplugin.services.sessionrecorder.SessionRecorderManager#createSessionRecorder()
     */
    @Override
    public SessionRecorder createSessionRecorder(CreateRecorderOption... options) {
        final SessionRecorderImpl sr = new SessionRecorderImpl(this.pluginManager);

        synchronized (this.allRecorder) {
            this.allRecorder.add(sr);
        }
        return sr;
    }
    ...
    public void shutdown() {
        synchronized (this.allRecorder) {
            for (final SessionRecorderImpl sr : this.allRecorder) {
                sr.shutdown();
            }
        }
    }
    ...
    public String[] getCapabilities() {
        return new String[] { "sessionrecorder:plainxml" };
    }
