...
public class TrainedPeakEmotionClassifier implements EmotionClassifier {
    ...
    public String getEmotion() {
        ...
        if (currEvents.isEmpty()) return null;
        ...
        if (avg <= thresholdBored) bored = true;
        else if (avg >= thresholdInterested) interested = true;

        // doubt or happy (have peaks)
        for (BrainTrackingEvent b : currEvents) {
            if (b.getValue("channel:furrow") >= thresholdDoubt) doubt = true;
            else if (b.getValue("channel:smile") >= thresholdHappy) happy = true;
        }
